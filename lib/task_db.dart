...
class DbHelper {
  static const String _DB_NAME = "TaskData";
  static Database _db;
  ...
  Future<Database> _initDb() async {
    ...
    String path = join(documentsDirectory.path, _DB_NAME + ".db");
  ...
  void _onCreate(Database db, int version) async {
    ...
    await db.execute(
        "CREATE TABLE " + _DB_NAME + "(id INTEGER PRIMARY KEY, name TEXT, seconds INTEGER )");
  }
  ...
  Future<int> insertTask(TaskData taskData) async {
    ...
    var count = Sqflite.firstIntValue(await dbClient.rawQuery(
        "SELECT COUNT(*) FROM " + _DB_NAME + " WHERE name = ?", [taskData.name]));
    ...
    // the task is not yet in the database
      ...
      taskData.id = await dbClient.insert(_DB_NAME, taskData.toMap());
      return taskData.id;
    }
  }
  ...
  Future<int> updateTask(TaskData taskData) async {
    ...
    // the number of SQL entries with the same name
    var count = Sqflite.firstIntValue(await dbClient.rawQuery(
        "SELECT COUNT(*) FROM " + _DB_NAME + " WHERE id = ?", [taskData.id]));
    // the task is already in the database
    if (count > 0) {
      // update the task data
      await dbClient.update(_DB_NAME, taskData.toMap(),
  ...
  void deleteTask(TaskData taskData) async {
    ...
    if (count > 0) {
      ...
      await dbClient
          .delete(_DB_NAME, where: "name = ?", whereArgs: [taskData.name]);
    }
  }
  ...
  Future<List<TaskData>> getTasks() async {
    var dbClient = await db;
    List<Map> results =
        await dbClient.query(_DB_NAME, columns: TaskData.columns);
    List<TaskData> taskDataList = List();
